...
vector<int> random_dimensions(const int size) {
    ...
    vector<int> rez;
    int new_size = size - 1;
    int max_val = floor(sqrt(size));
    int *aux_vec = new int[size];
    for (int i = 0; i <= new_size; ++i){
        aux_vec[i] = 0;
    }
    for (int i = 0; i < max_val; ++i){
        int rand_value = std::rand() % new_size + 1;
        if (aux_vec[rand_value] == 1){
            while (aux_vec[rand_value] != 0){
                rand_value = std::rand() % new_size + 1;
            }
        }
        aux_vec[rand_value] = 1;
        rez.push_back(rand_value);
    }
    delete[] aux_vec;
    return rez;
}
