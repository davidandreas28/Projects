...
final class Basic extends Player {
	...
	void createSackAndDeclaration() {
		...
		if (this.thereAreLegalItemsInHand()) {
			...
			for (int i = 0; i < this.tableOfFreq.getFreqTable().size(); ++i) {
				...
				} else if (max == this.tableOfFreq.getFreqTable().
						get(i).getFrequency()) {
					duplicates++;
					sameElements.add(this.tableOfFreq.
							getFreqTable().get(i).getType());
			...
			if (duplicates == 1) {
				...
				for (int i = 0; i < sameElements.size(); ++i) {
					for (int j = 0; j < this.tableOfFreq.
							getFreqTable().size(); ++j) {
						if (this.tableOfFreq.getFreqTable().get(j).
								getType().equals(
								sameElements.get(i))) {
	...
	protected void deleteFreqVector() {
	...
	void inspectSack(final Player player, final Assets assetTable) {
		if (player.getBribe() != 0) {
			player.addToCoins(player.getBribe());
			player.removeBribe();
		}
		if (player.lieCheck()) {
			int sum = player.countAssets()
					* player.getAssetsInSack().
					get(0).getPenalty();
