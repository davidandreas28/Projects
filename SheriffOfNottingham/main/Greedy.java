...
final class Greedy extends Player {
	...
	void createSackAndDeclaration() {
		...
		if (this.thereAreLegalItemsInHand()) {
			for (int i = 0; i < this.getAssetsInHand().size(); ++i) {
				if (this.getAssetsInHand().get(i).
						getType().equals("Legal")) {
					if (tableOfFreq.findInFreqTable(
						this.getAssetsInHand().get(i))) {
						tableOfFreq.getFreqTable().get(
						tableOfFreq.getPosOfDuplicate(
						this.getAssetsInHand(
						).get(i)))
						.copyFromAsset(this.
			...
			for (int i = 0; i < this.tableOfFreq.
					getFreqTable().size(); ++i) {
				if (max < this.tableOfFreq.getFreqTable().
						get(i).getFrequency()) {
					while (!(sameElements.isEmpty())) {
						sameElements.remove(0);
					}
					max = this.tableOfFreq.
							getFreqTable().
							get(i).getFrequency();
					duplicates = 1;
			...
			int maxProfitsIndex = -1;
			for (int i = 0; i < this.getAssetsInHand().size(); ++i) {
				if (maxProfits < this.getAssetsInHand().get(i).getProfit()) {
					maxProfits = this.getAssetsInHand().get(i).getProfit();
					maxProfitsIndex = i;
				}
			}
		...
		if ((this.getRound() % 2) == 0) {
			if ((this.getAssetsInSack().size() <= Constants.MAX_ALLOWED_SACK)
					&& (this.thereAreIllegalCards())) {
				int maxProfit = -1;
				int maxProfitIndex = -1;
				for (int i = 0; i < this.getAssetsInHand().size(); ++i) {
					if (maxProfit < this.getAssetsInHand().
							get(i).getProfit()) {
						maxProfit = this.getAssetsInHand().
								get(i).getProfit();
						maxProfitIndex = i;
					}
				}

				this.getAssetsInSack().add(
						this.getAssetsInHand().remove(maxProfitIndex));
			}
		}
		this.newRound();
	}

	void deleteFreqVector() {
		while (!tableOfFreq.getFreqTable().isEmpty()) {
			tableOfFreq.getFreqTable().remove(0);
		}
	}
	...
	void inspectSack(final Player player, final Assets assetTable) {
		if (player.getBribe() == 0) {
			if (player.lieCheck()) {
				int sum = player.countAssets()
						* player.getAssetsInSack().get(0).getPenalty();
				player.addToCoins(sum);
				this.payPenalty(sum);
				player.transferToAssetsOnMerchantStand(
						player.getAssetsOnMerchantStand());
			} else {
				for (int i = 0; i < player.getAssetsInSack().size(); ++i) {
					if (!(player.getAssetsInSack().get(i).
							getNameAsset().equals(
									player.
									getDeclaredAsset()))) {
						player.payToSheriff(player.
								getAssetsInSack().get(i));
						this.addToCoins(player.getAssetsInSack().
								get(i).getPenalty());
						Asset confiscatedAsset = player.
								getAssetsInSack().get(i);
						assetTable.getAssets().add(confiscatedAsset);
						player.getAssetsInSack().remove(i);
						i--;
					}
				}
				player.transferToAssetsOnMerchantStand(
