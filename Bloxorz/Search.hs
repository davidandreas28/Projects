...
data Node s a = NullNode | MakeNode {
    state :: s,
    lastAction :: Maybe a,
    parentNode :: (Node s a),
    depth :: Int,
    successorsHere :: [Node s a]
} deriving (Eq)
...
-}
generateSuccs :: (ProblemState s a) => Node s a -> [(a, s)] -> [Node s a]
generateSuccs _ [] = []
generateSuccs currentParent currentSuccessors = currentNode : (generateSuccs currentParent (tail currentSuccessors))
    where
        currentNode = MakeNode p1 p2 p3 p4 p5
        p1 = snd (head currentSuccessors)
        p2 = Just (fst (head currentSuccessors))
        p3 = currentParent
        p4 = (depth currentParent) + 1
        p5 = generateSuccs currentNode (successors p1)

createStateSpace :: (ProblemState s a) => s -> Node s a
createStateSpace firstState = root
    where root = MakeNode firstState Nothing NullNode 0 (generateSuccs root (successors firstState))
